describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for(let i = 0; i < prodItemsData.length; i++){
      ...
      console.log(`Checking product item ${i + 1}/${prodItemsData.length}`, product);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cardCountHandle = await page.$('#cart-count');
    const cardCountText = await (await cardCountHandle.getProperty('innerText')).jsonValue();

    expect(cardCountText).toBe("20")
